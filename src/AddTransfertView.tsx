...
class AddTransactionView extends React.Component<Props, State> {
  ...
  public changeNumber(text: string, decimals: number): {text: string, number: number} {
  ...
  public changePriceTo(text: string) {
    const result = this.changeNumber(text, 2);
  ...
  public changeWalletUUIDFrom(text: string) {
    const result = this.changeNumber(text, 2);
    this.setState({...this.state, From : {...this.state.From, WalletUUID : text}});
  }
  public changeWalletUUIDTo(text: string) {
    const result = this.changeNumber(text, 2);
  ...
  public render() {
    ...
    return (
      ...
      outerContainerStyles={{height: 60}}
        ...
        rightComponent={this.props.TransfertUUID ? <View /> : <MyLink to={`AddTransactionView`} replace><Icon name="insert-drive-file" /></MyLink>}
      />
        {content}
      </View>
    );
